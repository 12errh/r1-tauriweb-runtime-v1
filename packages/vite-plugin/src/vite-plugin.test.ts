...
describe('Vite Plugin - Phase 1 Tests', () => {
  ...
  it('patches all tauri sub-path imports', () => {
    ...
    expect(output1).toContain('@r1/apis/fs');
    ...
    expect(output2).toContain('@r1/apis/path');
    ...
    expect(output3).toContain('@r1/apis/event');
    ...
    expect(output4).toContain('@r1/apis/window');
    ...
    expect(output5).toContain('@r1/apis/core');
    ...
    expect(output6).toContain('@r1/apis/core');
    ...
    expect(output7).toContain('@r1/apis/store');
    ...
    expect(output8).toContain('@r1/apis');
  ...
  it('patcher handles both single and double quotes', () => {
    ...
    expect(patchImports(singleQuote)).toContain("'@r1/apis/fs'");
    expect(patchImports(doubleQuote)).toContain('"@r1/apis/fs"');
  });
  ...
  it('patcher is idempotent', () => {
    ...
    expect(firstPass).toContain('@r1/apis/fs');
  });
  ...
  it('patcher processes longer paths before shorter paths', () => {
    ...
    // This should become @r1/apis/fs, not @r1/apis (which would happen if bare path is processed first)
    const input = `import { readDir } from '@tauri-apps/api/fs'`;
    ...
    expect(output).toBe(`import { readDir } from '@r1/apis/fs'`);
  });
});
