...
/// if it's out of date.
#[cfg(not(target_arch = "wasm32"))]
pub fn initialize_database(app_handle: Option<&AppHandle>) -> Result<Connection, rusqlite::Error> {
    let sqlite_path = {
        let handle = app_handle.expect("AppHandle required for native build");
        let app_dir = handle.path_resolver().app_data_dir().expect("The app data directory should exist.");
        fs::create_dir_all(&app_dir).expect("The app data directory should be created.");
        app_dir.join("MyApp.sqlite")
    };
...
pub fn initialize_database(_app_handle: Option<&()>) -> Result<Connection, rusqlite::Error> {
    let sqlite_path = "/app/data/MyApp.sqlite";
    let mut db = Connection::open(sqlite_path)?;
