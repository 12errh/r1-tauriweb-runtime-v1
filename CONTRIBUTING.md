...
R1 is at v0.3 and all packages are published to npm and crates.io. The most valuable thing you can do right now is **test it with your own Tauri app** and report what breaks.
...
Take a Tauri app — yours or an open source one — and try to run it through R1:
...
A detailed bug report is more valuable than a PR at this stage.
...
# 4. Run the test suite — all 105 should pass
npm test
...
3. Make sure all 105 tests still pass:
...
**What actually happened** — the exact error message, including the full browser console output or terminal output.
...
- Keep packages independent — `@r1-runtime/kernel` must never import from `@r1-runtime/core`, and `@r1-runtime/core` must never import from `@r1-runtime/apis`
